...
public function __construct(Servicegeo $Servicegeo)
    ...
    // {
    //     $reservationsapprouve = $repository->findBy(['id_utilisateur' => $user]);
    ...
    // }
    #[Route('/calen', name: 'app_calen', methods: ['GET'])]
    public function calen(CovoiturageRepository $repository, UserInterface $user)
    {
        $user = $this->getUser();

        $reservations = $repository->findBy(['id_utilisateur' => $user]);
        ...
        foreach ($reservations as $reservation) {
            $events[] = [
                ...
                'start' => $reservation->getDateCovoiturage()->format('Y-m-d H:i:s'),
                'end' => $reservation->getDateCovoiturage()->format('Y-m-d H:i:s'),
                'title' => $reservation->getDestination(),
                'backgroundColor' => 'green',
            ];
        }
...
public function index(CovoiturageRepository $covoiturageRepository, Request $request): Response
{
    ...
    // Get all covoiturages except those created by the current user
    ...
    // Si le tri ascendant est sélectionné
    if ($triAscendant) {
        $covoiturages = $covoiturageRepository->findBydateDsc();
    }
...
// return $this->render('covoiturage/statistique.html.twig');
    ...
    public function indexM(CovoiturageRepository $covoiturageRepository): Response
    {
      ...
      if ($this->isGranted('ROLE_ADMIN')) {
        ...
        // Get the covoiturages of the authenticated user
        $covoiturages = $covoiturageRepository->findBy(['id_utilisateur' => $user]);
    ...
    {
        $search = $request->query->get('searchcov');
...
#[Route('/get-address/{lat}/{lng}', name: 'getadress')]
    ...
    public function add(Request $request ,UserRepository $repo): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $user = $this->getUser();
             $nom = $repo->find($user)->getNom() ; 
             $telephone = $repo->find($user)->getTelephone() ; 
$doctor->setNom($nom) ; 
$doctor->setTelephone($telephone) ; 

        $doctor->setIdUtilisateur($user); 
...
//     }
    ...
    #[Route('suu/{id}', name: 'app_covoiturage_delete', methods: ['POST'])]
